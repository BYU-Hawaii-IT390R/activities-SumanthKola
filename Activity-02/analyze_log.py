...
def analyze_failed_logins(path: str, min_count: int):
    ...
    # Manually filter IPs with enough failed attempts
    ...
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            if match:
                ...
                # Remove literal b'' from the values
                user = user.strip("b'\"")
                pw = pw.strip("b'\"")
                ...
                # If values are byte-like, decode them
                if isinstance(user, bytes):
                    user = user.decode("utf-8")
                if isinstance(pw, bytes):
                    pw = pw.decode("utf-8")

                # Add IP to the (user, pw) pair
                cred_map[(user, pw)].add(ip)
...
# ── Extra Credit: Top Shell Commands ────────────────────────────────────────

# ChatGPT-assisted regex and logic
def top_commands(path: str):
    ...
    _print_counter(command_counter, "Command", "Count")
...
def main():
    ...
    parser.add_argument("--task",
                        required=True,
                        choices=["failed-logins", "connections", "successful-creds", "identify-bots", "top-commands"],
